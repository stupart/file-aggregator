    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          Manage Presets
          <Button startIcon={<AddIcon />} onClick={handleCreateNew} disabled={isCreating || !!editingPreset}>
            Create New
          </Button>
        </Box>
      </DialogTitle>
            ...
            {presets.map((preset) => (
              <React.Fragment key={preset.id}>
                <ListItem>
                  ...
                  <ListItemSecondaryAction>
                    <Tooltip title="Edit">
                      <IconButton edge="end" onClick={() => handleEdit(preset)} sx={{ mr: 1 }}>
                        <EditIcon />
                      </IconButton>
                    </Tooltip>
      ...
      <DialogActions>
        {/* Changed from "Close" to "Save Preset" */}
        <Button onClick={onClose}>Save Preset</Button>
      </DialogActions>
    </Dialog>
  );
};